btnConvertir.addEventListener("click", async function() {
    ...
    try {
        const data = await ObtenerApi(`https://mindicador.cl/api/${selectValue}`); 
        console.log(data);
...
async function getAndCreateDataToChart(data) {
...
async function renderGrafica(data) {
    const chartData = await getAndCreateDataToChart(data);
